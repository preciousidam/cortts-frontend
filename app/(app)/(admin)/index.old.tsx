        ...
        <View style={styles.container}>
          {!isMobile && <View style={styles.header}>
            <View style={{ rowGap: heightPixel(4), flex: 1 }}>
              ...
              <Typography size='body' color={colors.text.weak}>Welcome to Cortts project, here is your progress summary</Typography>
            </View>
            <PopupMenuV1
              anchor={
                ({ref, onPress}) => <Button
          ...
          <View style={styles.revenue}>
            ...
            <View style={[styles.revenueCard, isMobile && {width: '100%'}]}>
              ...
              <Typography variant='bold' size={isMobile ? 'subtitle' : 'h2'} color={colors.error.normal}>{Intl.NumberFormat('en-NG', { style: 'currency', currency: 'NGN' }).format(data?.total_outstanding ?? 0)}</Typography>
              <Typography variant='regular' size='caption'>Number of outstanding payments across all units.</Typography>
            </View>
          </View>
          <View style={styles.stats}>
            <StatCard title='Total Projects' value={data?.total_projects ?? 0} icon={<UnitSVG width={widthPixel(21)} height={heightPixel(21)} />} backgroundColor={colors.primaryBlue.lightActive} iconBgColor='#C5E8FF' />
            <StatCard title='Total Units' value={data?.total_units ?? 0} icon={<AntDesign name='home' size={fontPixel(21)} />} backgroundColor={colors.secondaryGreen.lightActive} iconBgColor='#74FFE9' />
            <StatCard title='Total Clients' value={data?.total_users ?? 0} icon={<UsersSVG width={widthPixel(21)} height={heightPixel(21)} />} backgroundColor={colors.error.lightActive} iconBgColor='#FFBBA9' />
            <StatCard title='Payment Logged' value={data?.total_payments ?? 0} icon={<PaymentSVG width={widthPixel(21)} height={heightPixel(21)} />} backgroundColor={colors.warning.lightActive} iconBgColor='#FCFC98' />
          </View>
